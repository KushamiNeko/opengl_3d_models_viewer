...
static int ui_piemenu(struct nk_context *ctx, struct nk_vec2 pos, float radius,
  ...
  if (nk_popup_begin(
          ctx, &popup, NK_POPUP_STATIC, "piemenu", NK_WINDOW_NO_SCROLLBAR,
          nk_rect(pos.x - total_space.x - radius,
                  pos.y - radius - total_space.y, 2 * radius, 2 * radius))) {
    int i = 0;
    ...
    nk_fill_circle(out, bounds, nk_rgb(50, 50, 50));
    {
      ...
      struct nk_vec2 center =
          nk_vec2(bounds.x + bounds.w / 2.0f, bounds.y + bounds.h / 2.0f);
      struct nk_vec2 drag =
      ...
      for (i = 0; i < item_count; ++i) {
        ...
        nk_stroke_line(out, center.x, center.y, center.x + dx, center.y + dy,
                       1.0f, nk_rgb(50, 50, 50));
        ...
        content.w = 30;
        content.h = 30;
        content.x = center.x + ((rx * (float)cos(a) - ry * (float)sin(a)) -
                                content.w / 2.0f);
        content.y = center.y + (rx * (float)sin(a) + ry * (float)cos(a) -
                                content.h / 2.0f);
...
static void grid_demo(struct nk_context *ctx, struct media *media) {
  ...
  if (nk_begin(ctx, &layout, "Grid Demo", nk_rect(600, 350, 275, 250),
    ...
    nk_edit_string(ctx, NK_EDIT_FIELD, text[0], &text_len[0], 64,
                   nk_filter_float);
    nk_label(ctx, "Hexadecimal:", NK_TEXT_RIGHT);
    nk_edit_string(ctx, NK_EDIT_FIELD, text[1], &text_len[1], 64,
                   nk_filter_hex);
    nk_label(ctx, "Binary:", NK_TEXT_RIGHT);
    nk_edit_string(ctx, NK_EDIT_FIELD, text[2], &text_len[2], 64,
                   nk_filter_binary);
    nk_label(ctx, "Checkbox:", NK_TEXT_RIGHT);
    nk_checkbox_label(ctx, "Check me", &check);
    nk_label(ctx, "Combobox:", NK_TEXT_RIGHT);
    if (nk_combo_begin_label(ctx, &combo, items[selected_item],
 ...
 *    *
 *     * ===============================================================*/
static void ui_header(struct nk_context *ctx, struct media *media,
                      const char *title) {
  nk_style_set_font(ctx, &media->font_18->handle);
...
static void ui_widget(struct nk_context *ctx, struct media *media,
                      float height) {
...
static void ui_widget_centered(struct nk_context *ctx, struct media *media,
                               float height) {
...
static void button_demo(struct nk_context *ctx, struct media *media) {
  ...
  nk_menubar_begin(ctx);
  {
    ...
    if (nk_menu_begin_image(ctx, &menu, "Music", media->play,
                            nk_vec2(110, 120))) {
      /* settings */
      nk_layout_row_dynamic(ctx, 25, 1);
   ...
   *           *------------------------------------------------*/
  nk_style_set_font(ctx, &media->font_18->handle);
  if (nk_contextual_begin(ctx, &menu, NK_WINDOW_NO_SCROLLBAR, nk_vec2(150, 300),
                          nk_window_get_bounds(ctx))) {
    nk_layout_row_dynamic(ctx, 30, 1);
    if (nk_contextual_item_image_label(ctx, media->copy, "Clone",
                                       NK_TEXT_RIGHT))
      fprintf(stdout, "pressed clone!\n");
    if (nk_contextual_item_image_label(ctx, media->del, "Delete",
                                       NK_TEXT_RIGHT))
      fprintf(stdout, "pressed delete!\n");
    if (nk_contextual_item_image_label(ctx, media->convert, "Convert",
                                       NK_TEXT_RIGHT))
...
static void basic_demo(struct nk_context *ctx, struct media *media) {
  ...
  if (image_active) {
    struct nk_panel popup;
    if (nk_popup_begin(ctx, &popup, NK_POPUP_STATIC, "Image Popup", 0,
  ...
  ui_widget(ctx, media, 40);
  if (nk_combo_begin_image_label(ctx, &combo, items[selected_icon],
                                 media->images[selected_icon],
                                 nk_vec2(nk_widget_width(ctx), 200))) {
    nk_layout_row_dynamic(ctx, 35, 1);
    for (i = 0; i < 3; ++i)
      if (nk_combo_item_image_label(ctx, media->images[i], items[i],
                                    NK_TEXT_RIGHT))
        selected_icon = i;
    nk_combo_end(ctx);
  }
